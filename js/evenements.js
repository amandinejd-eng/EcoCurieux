  ...
  const mainPhotoHtml = event.photos && event.photos.length > 0 ? 
    (() => {
      ...
      const captionHtml = caption ? 
        `<div class="photo-caption" style="position:absolute;bottom:0;left:0;right:0;background:rgba(70,123,67,0.85);color:white;padding:1rem;font-family:'Montserrat',sans-serif;font-size:0.9rem;font-weight:600;text-align:center;transform:translateY(100%);transition:transform 0.4s ease;z-index:10;">${caption}</div>` 
        : '';
        
      return `<div style="width:100%;height:280px;border-radius:12px;margin-bottom:1.5rem;box-shadow:0 4px 15px rgba(0,0,0,0.08);position:relative;cursor:pointer;overflow:hidden;" 
        ...
        <img src="${photoUrl}" alt="${caption || event.title}" style="width:100%;height:100%;object-fit:cover;transform:rotate(${rotation}deg);transition:transform 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);position:relative;z-index:1;">
        ${captionHtml}
      </div>`;
    })()
    : '';
